...
import { apiConfirmOrder, apiEndOrder, apiPaidOrder, apiPrepaid, apiStartOrder, getInfor } from '../../api';
...
import ConfirmOrderPopup, { IConfirmPopupRef } from '../ConfirmOrderPopup';
import { onPushEventBus, EventBusName } from 'services/event-bus';
...
const handleConfirmOrder = async (orderId: number, action: string) => {
  try {
    showLoading();
    const response = await apiConfirmOrder(orderId, action);
    hideLoading();
    if (response) {
      onPushEventBus(EventBusName.ACTION_ORDER)
    }
    showCustomToast(action === 'accept' ? `Bạn đã nhận tour thành công` : `Bạn đã từ chối tour`);
  } catch (error) {
    hideLoading();
...
const TourGuideTourStatusItem = (props: TourStatusItemProps) => {
  ...
  const renderButtonLeft = () => {
    if (order.status === EOrderStatus.WAITING_TOUR_GUIDE) {
      actionButton = {
        title: 'Bạn có muốn nhận dẫn chuyến đi này ?',
        onConfirm: () => handleConfirmOrder(order.id, 'accept'),
        onReject: () => handleConfirmOrder(order.id, 'reject'),
        text: `Bạn có muốn nhận chuyến đi trong khoảng thời gian: ${order.startDate.split('-').reverse().join('-')} đến ${order.endDate.split('-').reverse().join('-')}`,
      };
      return (
        <>
          <TouchableOpacity style={styles.shopContainer} onPress={refConfirmOrderPopup?.current?.showModal}>
            <SvgIcons.IcShopOutline color={getThemeColor().white} width={scales(17)} height={scales(17)} />
            <Text style={styles.sellNow}>Phản hồi</Text>
          </TouchableOpacity>
        </>
      );
      ...
      actionButton = {
        ...
        text: `Bắt đầu tour du lịch: ${tour.name}`,
      };
      return (
...
export default TourGuideTourStatusItem;
